...
export async function runApiExecutePlanCommand(uniqueNameArg: string | undefined, simulate: boolean, jsonOutput: boolean): Promise<void> {
  ...
  for (const operation of result.executionState.operations) console.log(`- [${operation.sequence}] ${operation.action} ${operation.uniqueName}: ${operation.status}`);
}
