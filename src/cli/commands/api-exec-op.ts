...
export async function runApiExecuteOperationCommand(uniqueNameArg: string | undefined, operationId: string, simulate: boolean, jsonOutput: boolean): Promise<void> {
  const result = await executeCustomApiSyncOperation(operationId, uniqueNameArg, simulate);
  ...
  if (result.result.error) {
    ...
    if (result.result.error.details) console.log(`Details: ${result.result.error.details}`);
  }
}
